...
/**
 * Plugin Name: CH2 - Page Header Output Metaboxes
 * Plugin URI:
 * Description: Output content in page Head. Admin Metaboxes
 * Version: 1.0
 * Author: Mauro Bono
...
function ch2pho_help_tabs()
{
  ...
  $screen->add_help_tab($instructions_args);
  $screen->add_help_tab($faq_args);
  $screen->set_help_sidebar('<p>This is the sidebar content</p>');

  global $options_page;

  add_meta_box(
    'ch2pho_general_meta_box',
    'General Settings',
    'ch2pho_plugin_meta_box',
    $options_page,
    'normal',
    'core'
  );
...
{ ?>
  ...
  Track Outgoing Links <input type="checkbox" name="track_outgoing_links" <?php checked($options['track_outgoing_links']); ?> /><br />
<?php }
